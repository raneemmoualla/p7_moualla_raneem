    ...
    <main class="container">  
        ...
        <div class="row">
            ...
            <article id="panelComment" class="col-12 col-md-4" > 
                <div class="card mb-3">
                    ...
                    <div class="justify-content-center align-self-center">
                        <router-link to='/Create' ><button type="button" class="btn btn-info  rounded m-2 p-2 font-weight-bold ">CREER UNE PUBLICATION<img src="../assets/send.svg" alt="logo-envoyer"  style="width: 25px" class="m-3" />  </button></router-link>
                    </div>
                    <div class="justify-content-center align-self-center">
                        <router-link to='/CreateComment'><button type="button" class="btn btn-info  m-2 p-2 rounded font-weight-bold" >COMMENTER<img src="../assets/write.svg" alt="write-logo"  style="width: 25px" class="m-3" ></button></router-link>
                    </div>
                </div>
            </article>
            <!--Section message -->
            <section class="card col-12 col-md-8 bg-light mb-3">
                <div class="card-header bg-light d-flex align-items-center justify-content-between m-0 p-1">
                    ...
                    <div class="badge bg-secondary text-wrap text-white p-2" style="width: 6rem;">
                        ref # {{ messageId}}
                    </div>                                                                                       
                </div>
                <div>
        ...
        <section class="row">
            ...
            <div v-for="comment in comments" :key="comment" class="card col-12 mt-3">
                <div class="card-header ">
                    <p class="btn btn-secondary w-100" style="cursor:default">Commentaire rédigé le {{ comment.createdAt.slice(0,10).split('-').reverse().join('/') }}  par {{ comment.User.userName.charAt(0).toUpperCase() + comment.User.userName.slice(1) }}</p>
                </div>
                <div class="card-body border bg-info text-white text-right rounded mx-2">
                    <p>  {{ comment.comment }} </p>
                </div>
                <div class="card-footer row justify-content-between">
                    <p class="btn btn-secondary w-10 text-right mx-2 " style="cursor:default">ref# <span class="badge badge-light"> {{ comment.id }}</span></p>
